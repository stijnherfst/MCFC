...
export class LearnSpell {
	...
	learnSpell() {
		...
		let ability = spellToAbility.get(itemID)
		let player = this.players[GetPlayerId(GetOwningPlayer(GetTriggerUnit()))]

		if (player.abilities.includes(ability)) {
			DisplayTimedTextToPlayer(GetTriggerPlayer(), 0, 0, 10.00, "You've already read this spell book before.")
			CreateItem(itemID, GetUnitX(GetTriggerUnit()), GetUnitY(GetTriggerUnit()))
			return
		}

		if (player.abilities.length >= maxAbilities) {
			DisplayTimedTextToPlayer(GetTriggerPlayer(), 0, 0, 10.00, "You do not have the capacity to remember an 8th spell. Maybe you have to forget one first?")
			CreateItem(itemID, GetUnitX(GetTriggerUnit()), GetUnitY(GetTriggerUnit()))
			return
		}
